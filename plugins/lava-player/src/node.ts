...
export function getNode(client: Client): Node {
  const nodeX = new Node({
    ...
    shardCount: process.env.LAVA_SHARD ? Number(process.env.LAVA_SHARD) : 0, // the total number of shards that your bot is running (optional, useful if you're load balancing)
